...
TEST_CASE("postgresql test single select statement", "[pg]"){
    ...
    CHECK((pg::perform<students::all>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students"_s));
    CHECK((pg::perform<students::ordered_by_marks>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students order by students.marks asc"_s));
    CHECK((pg::perform<students::top5>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students order by students.marks desc limit $1 offset $2"_s));
    CHECK((pg::perform<students::top5_names>(io, pool)->sql().text() == "select students.name from students order by students.marks desc limit $1 offset $2"_s));
    CHECK((pg::perform<students::top5_short>(io, pool)->sql().text() == "select students.id, students.name from students order by students.marks desc limit $1 offset $2"_s));
    CHECK((pg::perform<students::top5_marks_str>(io, pool)->sql().text() == "select students.id, students.name, CAST(students.marks as text) from students order by students.marks desc limit $1 offset $2"_s));
    
    CHECK((pg::perform<students::by::id>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students where students.id = $1"_s));
    CHECK((pg::perform<students::by::project>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students where students.project = $1"_s));
    CHECK((pg::perform<students::qualified>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students where students.marks >= $1 order by students.marks desc"_s));
    CHECK((pg::perform<students::disqualified>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students where students.marks < $1 order by students.marks asc"_s));
    CHECK((pg::perform<students::by::name>(io, pool)->sql().text() == "select students.id, students.name, students.project, students.marks from students where students.project = $1 and students.name like $2"_s));
    
    CHECK((pg::perform<students::top5_above_cutoff>(io, pool)->sql().text() == "select students.id, students.name, students.marks from students where students.marks >= $1 order by students.marks desc limit $2 offset $3"_s));

    CHECK((pg::perform<students::detailed>(io, pool)->sql().text() == "select students.id, students.name, COUNT(memberships.project) as projects_associated, COUNT(articles.id) as articles_published from students inner join articles on students.id = articles.author  inner join memberships on students.id = memberships.student"_s));
}
