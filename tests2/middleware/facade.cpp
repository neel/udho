...
TEST_CASE("Facade construction and retrieval") {
    ...
    SECTION("Component retrieval by feature and index") {
        REQUIRE(&facade.get<test::feature_a, 0>().component() == &a1);
        REQUIRE(facade.get<test::feature_b, 0>().component().data == "moved");
    }
...
TEST_CASE("Evaluation workflow") {
    ...
    auto facade = udho::middleware::facade<test::component_a, test::component_b>(a, std::move(b));
    auto states = udho::middleware::states<test::component_a, test::component_b>();
